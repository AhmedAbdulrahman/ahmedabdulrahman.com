...
yarn add react react-dom gatsby web-template
```
...
### Step 3: Using `web-template`

You must add `web-template` as a plugin in `gatsby-config.js`.
`web-template` is configurable through plugin options but we will use the defaults for now.

```js
// gatsby-config.js

module.exports = {
  plugins: ["web-template"],
};
```

---

Once you've setup `web-template` plugin in `gatsby-config.js` you can start creating your first Posts. In order to create a Post you also need at least one Author.
...
Once you have your project setup with `gatsby-source-contentful` and `web-template` and your Contentful space is setup with the imported model you are good to go.
...
import novelaTheme from "web-template/src/gatsby-plugin-theme-ui";
