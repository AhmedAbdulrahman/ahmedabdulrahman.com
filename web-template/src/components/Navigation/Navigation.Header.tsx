...
const DarkModeToggle: React.FC<{ siteUrl: string }> = ({ siteUrl }) => {
	...
	const handleColorMode = (event): void => {
...
const SharePageButton = (): JSX.Element => {
...
const NavigationHeader = (): JSX.Element => {
	...
	const [previousPath, setPreviousPath] = useState<string | null>('/writing')
	...
	const isDark = colorMode === `dark`
	const fill = isDark ? '#fff' : '#212529'
	const fillOut = isDark ? '#000' : '#fff'
	const { rootPath, basePath, writingPath } = sitePlugin.pluginOptions

	useEffect(() => {
		const { width }: { width: number } = getWindowDimensions()
		const phablet: number = getBreakpointFromTheme('phablet')
		...
		>
			<NavContainer>
				...
				>
					...
					<Logo fill={fill} fillOut={fillOut} />
					<Hidden>Navigate back to the homepage</Hidden>
				</LogoLink>
				<NavControls>
					{showBackArrow && previousPath ? (
						<button
								...
								<>
									...
									<NavContent burgerOpen={burgerOpen}>
										<MobileMenu burgerOpen={burgerOpen}>
											<NavItem>
												<NavLink to={`/writing`} title={`All Article`} activeClassName="active">
...
const MobileMenu = styled.ul<{ burgerOpen: boolean }>`
	...
	background: #16161a;
	transform: ${(props) => (props.burgerOpen ? 'translateX(0)' : 'translateX(-150%)')};
`
...
const MoonOrSun = styled.div<{ isDark: boolean }>`
	...
	&::after {
		...
		box-shadow: 0 -23px 0 ${(p) => p.theme.colors.primary},
			0 23px 0 ${(p) => p.theme.colors.primary}, 23px 0 0 ${(p) => p.theme.colors.primary},
			-23px 0 0 ${(p) => p.theme.colors.primary}, 15px 15px 0 ${(p) => p.theme.colors.primary},
