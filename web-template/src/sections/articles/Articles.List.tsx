...
const ArticlesList: React.FC<ArticlesListProps> = ({ articles, alwaysShowAllDetails }) => {
	 ...
	 */
	const articlePairs = articles.reduce((result: any, _value, index, array) => {
		if (index % 2 === 0) {
...
const listTile = (p: { reverse: boolean }) => css`
	position: relative;
	display: grid;
...
const Title = styled(Headings.h2)<{
	...
	margin-bottom: ${(p) => (p.hasOverflow && p.gridLayout === 'tiles' ? '10px' : '10px')};
